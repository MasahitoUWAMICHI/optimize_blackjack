...
class BlackjackGEKKO(Blackjack):
    ...
    def init_value_cards_gekko(self, dict_key):
        ...
        if state_dict['num_residual_cards'] == 0:
            self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(state_dict['win'])
        ...
        if state_dict['score_bust_h']:
            self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(0)
            self.state_dict[dict_key]['Arrays_idx'] = None
            return
        if state_dict['score_bust_d']:
            self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(1)
            self.state_dict[dict_key]['Arrays_idx'] = None
            return
        ...
        if state_dict['c'] == 0:
            if state_dict['num_residual_cards'] == 0:
                ...
                self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.if3(self.q[self.variables_idx[dict_key]], self.m.sum(self.Arrays[-1]), self.m.value_cards_gekko[self.states_idx_dict[self.get_dict_key(state_dict['h'], state_dict['d'], 1)]])
                self.state_dict[dict_key]['Arrays_idx'] = len(self.Arrays) - 1
            return
        else:
            state_win = self.state_dict[dict_key]['win']
            if state_dict['num_residual_cards'] == 0:
                self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(state_win)
            elif state_win == 0:
                self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(state_win)
            else:
                self.m.value_cards_gekko[self.states_idx_dict[dict_key]] = self.m.Const(np.sum([self.value_cards(state_dict['h'], dp, state_dict['c'], lambda h,d,c: 1) * p_i for dp, p_i in zip(state_dict['d_plus'], state_dict['p'])]))
            self.state_dict[dict_key]['Arrays_idx'] = None
            return 
    ...
    def init_gekko(self):
        ...
        for i, q_i in enumerate(self.variables):
            self.q[i] = self.m.Var(value=q_i, lb=0, ub=1, integer=True)
    ...
    def optimize_gekko(self, display=True):
        # return the value of the game when the player follows the policy q
        self.m.Maximize(1 - self.m.value_cards_gekko[self.states_idx_dict[self.get_dict_key(np.zeros_like(self.t), np.zeros_like(self.t), 0)]])
        solver = 'ipopt'
